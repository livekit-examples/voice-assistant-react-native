import { StyleSheet, View, Text, useColorScheme, Image, Pressable, ScrollView } from 'react-native';
...
const RoomView = () => {
  ...
  // Transcriptions
  const localTracks = useParticipantTracks([Track.Source.Microphone], localParticipant.identity);
  const { segments: userTranscriptions } = useTrackTranscription(localTracks[0]);

  const { agentTranscriptions } = useVoiceAssistant();

  const lastUserTranscription = (userTranscriptions.length > 0 ? userTranscriptions[userTranscriptions.length - 1].text : "") as string
  const lastAgentTranscription = (agentTranscriptions.length > 0 ? agentTranscriptions[agentTranscriptions.length - 1].text : "") as string
...
const UserTranscriptionText = (props: { text: string }) => {
  ...
  const themeStyle = colorScheme === 'light' ? styles.userTranscriptionLight : styles.userTranscriptionDark;
  const themeTextStyle = colorScheme === 'light' ? styles.lightThemeText : styles.darkThemeText;

  return (
    text && <View style={styles.userTranscriptionContainer}>
...
const AgentTranscriptionText = (props: {text: string}) => {
  ...
  const themeTextStyle = colorScheme === 'light' ? styles.lightThemeText : styles.darkThemeText;
  return (
    text && <Text style={[styles.agentTranscription, themeTextStyle]}>
      {text}
    </Text>
  )
}
